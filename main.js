...
function addEmployee() {
  ...
  const chosen = prompt("Selecciona los procesos para este empleado (separa con coma):\nDock2, Dock3, Auditor, Cargas, Capitán");
  const selected = chosen
    ? chosen.split(",").map(p => p.trim().toLowerCase()).filter(p => defaultProcesses.includes(p))
    : [];

  if (selected.length === 0) return alert("Debes asignar al menos un proceso válido.");
